...
int main(int argc, const char * argv[]) {
    ...
    */
    ...
    cv::VideoCapture capture("rtsp://admin@10.0.10.6/snl/live/1/2");
    ...
    {
        cv::Mat frame, source;
        if ( ! capture.read(source) ) // another check !!
            break;
        ...
        // scale down image
        cv::resize(source, frame, Size(0,0), 0.33, 0.33);
        ...
        if (frameIndex == 0) {
            ...
            std::string filename = "./capture_old.avi";
        ...
        // deltaFrame.copyTo(deltaCopyFrame);
        cv::findContours(deltaFrame, contours, hierarchy, CV_RETR_TREE,
